...
import { convertToWebP, uploadToR2, r2KeyFromUrl } from "@/lib/r2";
import {
  ArtSchema,
  ART_PREVIEW_MIME,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const tagIds = tagIdsRaw
      .map((t: FormDataEntryValue) => (typeof t === "string" ? t.trim() : ""))
    ...
    const previewUrl = await uploadToR2(`arts/${Date.now()}-${name}.webp`, webpBuf, webpMime);
